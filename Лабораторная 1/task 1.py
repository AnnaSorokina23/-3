...
class Cloth:
    def __init__(self, amount_of_cloth: float):
        """
        ...
        """
        if not isinstance(amount_of_cloth, (int, float)):
            raise TypeError("Количество ткани должно быть типа int или float")
    ...
    def amount_short_dress(self, amount_for_short_dress: float) -> int:
        """
        ...
        :raise ValueError: Если имеющееся количество ткани меньше, чем требуется для короткого платья

        Примеры:
        >>> cloth = Cloth(2)
        >>> cloth.amount_short_dress(1)
        """
        if not isinstance(amount_for_short_dress, (int, float)):
            raise TypeError("Количество ткани должно быть типа int или float")
        if amount_for_short_dress <= 0:
            raise ValueError("Количество ткани должно быть положительным числом")
        ...

    def amount_long_dress(self, amount_for_long_dress: float) -> int:
        """
        Функция, которая считает сколько можно сшить коротких платьев

        :param amount_for_long_dress: Количество ткани необходимое для длинного платья

        :raise ValueError: Если имеющееся количество ткани меньше, чем требуется для длинного платья

        Примеры:
        >>> cloth = Cloth(2)
        >>> cloth.amount_long_dress(2)
        """
        if not isinstance(amount_for_long_dress, (int, float)):
            raise TypeError("Количество ткани должно быть типа int или float")
...
class Site:
    def __init__(self, site_length: float, site_width: float):
        """
        ...
        """
        if not isinstance(site_length, (int, float)):
            raise TypeError("Длина участка должна быть типа int или float")
        if site_length <= 0:
            raise ValueError("Длина участка должна быть положительным числом")
        self.site_length = site_length

        if not isinstance(site_width, (int, float)):
            raise TypeError("Ширина участка должна быть типа int или float")
        if site_width <= 0:
